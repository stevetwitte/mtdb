...
RSpec.describe Theory::Scale do
  let(:scale) { Theory::Scale.major('C') }
  ...
  it 'can be defined by interval steps' do
    expect(scale).to be_a(Theory::Scale)
  end
  ...
  it 'can give you a major scale' do
    expect(Theory::Scale.major('C').notes.names).to include('C', 'D', 'E', 'F', 'G', 'A', 'B')
    expect(Theory::Scale.major('C').notes.names).to_not include('C#', 'D#', 'F#', 'G#', 'A#')
    expect(Theory::Scale.major('D#').notes.names).to_not include('E')
  end

  it 'can tell you if a scale includes given notes' do
    expect(Theory::Scale.major('C').include?(Theory::NoteSet['A', 'B', 'C', 'D', 'E', 'F', 'G'])).to be_truthy
    expect(Theory::Scale.major('D#').include?(Theory::Note['E'])).to be_falsey
    expect(Theory::Scale.major('F#').include?(Theory::Note['G'])).to be_falsey
  end
  ...
  #   expect(Theory::Scale.pentatonic_minor.triads.map(&:name))
  #     .to include('')
  # end

  # it 'can give you all possible triads for pentatonic scale' do
  #   expect(Theory::Scale.pentatonic_major.sevenths.map(&:name))
  #     .to include('')
  # end
  ...
  it 'can return scales that include some notes' do
    scale_search = Theory::Scale.having_notes(Theory::NoteSet['C', 'F', 'B'])
  ...
  it 'can return the greek modes' do
    expect(Theory::Scale.ionian('C').notes.names).to     eq %w[C D E F G A B]
    expect(Theory::Scale.dorian('D').notes.names).to     eq %w[D E F G A B C]
    expect(Theory::Scale.phrygian('E').notes.names).to   eq %w[E F G A B C D]
    expect(Theory::Scale.lydian('F').notes.names).to     eq %w[F G A B C D E]
    expect(Theory::Scale.mixolydian('G').notes.names).to eq %w[G A B C D E F]
    expect(Theory::Scale.aeolian('A').notes.names).to    eq %w[A B C D E F G]
    expect(Theory::Scale.locrian('B').notes.names).to    eq %w[B C D E F G A]
    expect(Theory::Scale.ionian('C#').notes.map(&:letter)).to eq %w[C D E F G A B]
    expect(Theory::Scale.ionian('D').notes.map(&:letter)).to eq %w[D E F G A B C]

    letters = %w[C D E F G A B]
    %i[ionian dorian phrygian lydian mixolydian aeolian locrian].each do |mode|
      letters.each_with_index do |letter, index|
        expect(Theory::Scale.public_send(mode, letter).notes.map(&:letter))
        .to eq(letters.rotate(index)), -> {
          <<~ERROR
            failed on #{letter} #{mode}
            expected:   #{letters.rotate(index)}
            got:        #{Theory::Scale.public_send(mode, letter).notes.map(&:letter)}
            full_notes: #{Theory::Scale.public_send(mode, letter).notes.names}
          ERROR
        }
      end
    end
